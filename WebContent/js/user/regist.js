...
function validatereloginpass(){
	...
	if(reloginpass==null||reloginpass==""){
		$("#"+name+"Error").show();
		$("#"+name+"Error").text("确认密码不能为空");
	}else if(reloginpass!= $("#loginpass").val()){
		$("#"+name+"Error").show();
		$("#"+name+"Error").text("两次密码不一致");
	}
}
/**
 * Email不能为空；
		...
		$.ajax({
			...
			success:function(result){
				...
				if(!result){
					//说明提示用户，修改用户名
					$("#"+name+"Error").show();
					$("#"+name+"Error").text("请更换email，已经被占用！");
				}
			}
		});
	}*/
